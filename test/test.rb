...
class TestRubySDK < Test::Unit::TestCase
  ...
  def test_create_session
      assert_nothing_raised(OpenTok::OpenTokException) {
          ...
          xml = get_session_info(s.session_id, t)
...
# Commented out because "time" can end up being in the past because it takes time for the code to execute.
      # now
#      time = Time.now.to_i
#      t = @o.generate_token :session_id => s.session_id, :expire_time => time
#      xml = get_token_info t
...
#          'Ruby SDK tests: expire time not set'
  ...
  def get_session_info(session_id, token)
      ...
      res = http.post url.path + '?extended=true', '', headers
